...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function drawing(uint256 _externalRandomNumber) external onlyAdmin {
        ...
        // 2
        _structHash = keccak256(
            abi.encode(_blockhash, totalAmount, _gasleft, _externalRandomNumber)
        );
        _randomNumber = uint256(_structHash);
        assembly {
            _randomNumber := add(mod(_randomNumber, _maxNumber), 1)
        ...
        // 4
        _structHash = keccak256(
            abi.encode(_blockhash, _gasleft, _externalRandomNumber)
        );
        _randomNumber = uint256(_structHash);
        assembly {
            _randomNumber := add(mod(_randomNumber, _maxNumber), 1)
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        propellant.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if (reward > 0) {
            propellant.safeTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if (totalReward > 0) {
            propellant.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function adminWithdraw(uint256 _amount) public onlyAdmin {
        propellant.safeTransfer(address(msg.sender), _amount);
